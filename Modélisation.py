...
@author: chatoux
"""
#Code commenté par ANQUETIN Romain
...
def CameraCalibration():
    # Defini le nombre d'iteration et de l'erreur epsilon
    criteria = (cv.TERM_CRITERIA_EPS + cv.TERM_CRITERIA_MAX_ITER, 30, 0.001)
    # prepare object points, like (0,0,0), (1,0,0), (2,0,0) ....,(6,5,0)
    ############ to adapt ##########################
    objp = np.zeros((4*6, 3), np.float32) #changement de couleur entre noir et blanc
    objp[:, :2] = np.mgrid[0:4, 0:6].T.reshape(-1, 2)
    # Mise a l'echelle, 4cm par case et *40 pour etre en mm
    objp[:, :2] *= 40
    #################################################
    # Creation des tableaux
    objpoints = []  # Creation point changement de couleur
    imgpoints = []  # Creation point des angles
    ############ to adapt ##########################
    ...
    for fname in images:
        ...
        #################################################
        print(ret)
        # si les bords de l'echiquier sont trouves
        if ret == True:
            objpoints.append(objp)
            corners2 = cv.cornerSubPix(gray, corners, (11, 11), (-1, -1), criteria)
            imgpoints.append(corners)
            # Permet d'afficher les angles trouvés sur l'echiquier
            ############ to adapt ##########################
    ...
    # Trouve le parametre intrinseque et extrinseque
    ...
    # Calcul de la carte de non distortion et de rectification 
    mapx, mapy = cv.initUndistortRectifyMap(mtx, dist, None, newcameramtx, (w, h), 5)
    dst = cv.remap(img, mapx, mapy, cv.INTER_LINEAR)
    # Affiche la derniere image et attend une entree
    x, y, w, h = roi
    dst = dst[y:y + h, x:x + w]
    cv.namedWindow('img', 0)
    cv.imshow('img', dst)
...
def DepthMapfromStereoImages():
    ...
    speckleRange = 32)
    # Calcul la disparité à partir de 2 images proches
...
def StereoCalibrate(Cameramtx):
    ...
    # Detecte et calcul les points clées
    ...
    # Affiche les résultats pour savoir si l'algorithme fonctionne bien
    img3 = cv.drawMatches(img1,kp1,img2,kp2,good, None, flags=2)
    plt.imshow(img3)
    plt.show()

    # Calcul la matrice essentiel
    E, maskE = cv.findEssentialMat(pts1, pts2, Cameramtx, method=cv.FM_LMEDS)
    print('E\n',E)
    # Calcul de la position relative et de deplacement entre les 2 images
    ...
    Ktest = Cameramtx.reshape(3,3) # on utilise la matrice de la caméra calibrer pour trouver K
    ...
    #Calcul la matrice fundamentale
    F, maskF = cv.findFundamentalMat(pts1, pts2, cv.FM_LMEDS)
    print('F\n', F)
...
def EpipolarGeometry(pts1, pts2, F, maskF, FT, maskE):
    ...
    # Calcul des ligne epipolaire pour la premiere partie
    lines1 = cv.computeCorrespondEpilines(pts2.reshape(-1,1,2), 2,F)
    lines1 = lines1.reshape(-1,3)
    img5,img6 = drawlines(img1,img2,lines1,pts1F,pts2F)
    # Calcul les ligne epipolaire pour la deuxieme partie
    ...
    # Corrige les erreurs si la camera n'est pas bien calibre
    retval, H1, H2 = cv.stereoRectifyUncalibrated(pts1, pts2, F, (c,r))
    print(H1)
    print(H2)
    # Modifie la perspective pour que les images correspondes
